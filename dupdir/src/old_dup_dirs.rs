...
where
    P: AsRef<path::Path>,
{
    path.as_ref() != path::Path::new(".") && path.as_ref() != path::Path::new("") && path.as_ref() != path::Path::new("/Users/indiv0/Desktop/files")
}
...
impl Entries {
    ...
    fn dir_hash(&self, path: &path::Path) -> String {
        ...
        if dir.hash.is_none() {
            ...
            let hash = crate::hash_bytes(hashes.as_bytes());
            hash
        } else {
    ...
    fn dup_dirs(&self) -> Vec<(&path::Path, &str)> {
        ...
        println!("Found {} duplicate directories.", dupes.len());
        ...
        let dupes = dupes
            ...
            .filter(|(path, _hash)| {
                ...
                !ancestors
                    ...
                    .filter(valid_path)
                    .any(|ancestor| {
                        if dupes.contains_key(ancestor) {
                            //eprintln!("Skipping {path:?} because of ancestor {ancestor:?}");
                            true
                        } else {
                            false
                        }
        ...
        let dupes = dupes.collect::<collections::HashMap<_, _>>();
        println!("Found {} directories whose ancestors aren't also duplicates.", dupes.len());
...
pub fn main() {
    ...
    for file in files {
        ...
        while let Some(line) = lines.next() {
            let record = reader::read_line(&line);
    ...
    println!("Writing {} dir hashes to {}...", entries.dir_files.len(), "data_old/dirhashes.txt");
    let file = fs::File::create("data_old/dirhashes.txt").unwrap();
    ...
    println!("Writing duplicate directories to {} (sorted by hash)...", "data_old/dupdirs_by_hash.txt");
    dup_dirs.sort_by_key(|(_dir, hash)| *hash);
    let file = fs::File::create("data_old/dupdirs_by_hash.txt").unwrap();
    let mut file = io::LineWriter::new(file);
    ...
    println!("Writing duplicate directories to {} (sorted by path)...", "data_old/dupdirs_by_path.txt");
    dup_dirs.sort_by_key(|(dir, _hash)| *dir);
    let file = fs::File::create("data_old/dupdirs_by_path.txt").unwrap();
    let mut file = io::LineWriter::new(file);
